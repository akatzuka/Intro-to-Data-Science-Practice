...
vmode = function(x) {
  ...
  n = names(table(x)[table(x)==max(table(x))])
  return(n)
...
distances_between = function(x,y) {
  ...
  z = rbind(x,y)
  return(as.matrix(dist(z))[1:m,n:(n+m)])
...
knn_predict = function(knn, x, k = 5) {
  ...
  a = distances_between(x,knn$dat)
  b<-apply(a, 1, function(x) smallest_labels(x, knn$labels, k))
  c<-apply(b, 2, function(x) vmode(x))
  return(c)
